...
export default function Review({ productId }) {
	...
	const baseUrl = useMemo(() => process.env.REACT_APP_API_URL || 'http://localhost:5000', []);
		...
		// Load existing reviews with pagination
		useEffect(() => {
			let active = true;
			async function load() {
				setLoading(true);
	...
	// Check purchase eligibility using /api/orders/my (auth required)
	useEffect(() => {
		let active = true;
		...
		return () => { active = false; };
			...
			// Load current user's review (for edit/delete) separately
			useEffect(() => {
				let active = true;
					...
					try {
						...
						setMyReview(r || null);
							if (r) {
								...
								setEditing(true); // auto-enter edit mode if a review already exists
							} else {
								setEditing(false);
							}
					} catch {
		...
		setSubmitting(true);
		try {
						// If user already has a review, update it instead of creating a duplicate
						let url = `${baseUrl}/api/reviews`;
						let method = 'POST';
						if (myReview?._id) {
							url = `${baseUrl}/api/reviews/${myReview._id}`;
							method = 'PUT';
						}
								...
								<div className="text-xs text-gray-500 mt-1">{new Date(r.createdAt).toLocaleString()}</div>
										{user && (r.userId?._id === (user.id || user._id)) && (
											<div className="mt-2 flex gap-2">
												<button className="px-3 py-1 text-sm bg-gray-200 rounded" onClick={() => { setEditing(true); setForm({ rating: r.rating, comment: r.comment || '' }); setMyReview(r); window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' }); }}>Edit</button>
