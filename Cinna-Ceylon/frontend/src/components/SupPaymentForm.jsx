...
const COLORS = {
  PRIMARY: "#2A4D6E",      // Deep blue
  SECONDARY: "#4A7B9D",    // Medium blue
  ACCENT: "#FF9F43",       // Warm orange
  LIGHT: "#F8F9FC",        // Very light gray
  SUCCESS: "#10B981",      // Green
  WARNING: "#F59E0B",      // Amber
  ERROR: "#EF4444",        // Red
  TEXT: "#1F2937",         // Dark gray
  TEXT_LIGHT: "#6B7280",    // Medium gray
  SEA_BLUE: "#2079adff"      // Added sea blue color
};

const SupPaymentManagement = ({ onBackToDashboard }) => {
  ...
  const calculateNetPayment = (amount) => {
    ...
    const tax = amountValue * 0.05; // 5% tax
  ...
  const validateForm = () => {
    ...
    if (!formData.Amount || formData.Amount <= 0) {
      ...
      newErrors.Amount = 'Amount cannot exceed Rs. 10,000,000';
    }
    ...
    if (!formData.Date) {
      ...
      newErrors.Date = 'Date cannot be before 2020';
    }
  ...
  const handleDownloadPaymentPDF = async (paymentId) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/supplier-payments/${paymentId}/pdf`, {
        responseType: 'blob'
      });
  ...
  const filteredPayments = payments.filter(payment => {
    const matchesSearch = payment.Sup_id?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         payment.Sup_id?.contact?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen p-6" style={{ backgroundColor: '#E6F2FF' }}> {/* Light blue background */}
      <div className="max-w-7xl mx-auto">
        ...
        {/* Header Section with sea blue background */}
        <div className="rounded-2xl p-8 mb-8 shadow-md" style={{ backgroundColor: COLORS.SEA_BLUE, color: 'white' }}>
          <div className="mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-6">
            ...
            <StatCard
              ...
              value={`Rs. ${stats.averagePayment.toLocaleString()}`}
            ...
            <StatCard
              ...
              subtitle={`Rs. ${stats.thisMonthTotal.toLocaleString()}`}
              color={COLORS.SECONDARY}
              iconBg="bg-purple-500"
            />
          </div>

          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 bg-blue-50 p-4 rounded-xl" style={{ backgroundColor: 'rgba(255, 255, 255, 0.2)' }}>
            ...
            <div className="flex items-center gap-2">
              <FunnelIcon className="w-5 h-5" style={{ color: 'rgba(255, 255, 255, 0.96)' }} />
              <input
                type="date"
                value={filterDate}
                onChange={(e) => setFilterDate(e.target.value)}
                className="px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)',
                    color: '#000000'
                 }}
              />
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-lg hover:bg-gray-300 transition-colors text-sm font-medium"
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full p-3 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 ${
            ...
            <div>
              ...
              />
              <p className="mt-1 text-xs" style={{ color: COLORS.TEXT_LIGHT }}>Tax Rate: 5% of Amount</p>
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                className="w-full p-3 border border-gray-300 rounded-lg bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
              />
              <p className="mt-1 text-xs" style={{ color: COLORS.TEXT_LIGHT }}>Net Payment = Amount - Tax (5%)</p>
            </div>

            {/* Form Actions */}
            <div className="md:col-span-2 flex justify-end space-x-4 pt-6 border-t border-blue-200">
              <button 
                type="button" 
                onClick={resetForm} 
                className="px-5 py-2.5 bg-blue-200 rounded-lg hover:bg-gray-300 transition-colors flex items-center text-sm font-medium"
        ...
        <div className="bg-white rounded-2xl p-8 shadow-md" style={{ border: `1px solid ${COLORS.SECONDARY}20` }}>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold flex items-center" style={{ color: COLORS.PRIMARY }}>
              ...
              Supplier Payments
              <span className="ml-3 bg-blue-100 text-blue-800 px-2.5 py-0.5 rounded-full text-sm font-medium">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              {(searchTerm || filterDate) && (
                <button
                  onClick={() => { setSearchTerm(''); setFilterDate(''); }}
                  className="mt-4 px-4 py-2 text-white rounded-lg hover:shadow-md transition-all"
                  style={{ backgroundColor: COLORS.PRIMARY }}
                >
                  Clear Filters
                </button>
                    ...
                    <tr key={payment._id} className="hover:bg-gray-50 transition-colors duration-150">
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium" style={{ color: COLORS.TEXT }}>
                          {payment.Sup_id?.name || 'No Supplier Assigned'}
                        </div>
                        <div className="text-sm" style={{ color: COLORS.TEXT_LIGHT }}>
                          {payment.Sup_id?.contact || ''}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-sm" style={{ color: COLORS.TEXT }}>
                        Rs. {parseFloat(payment.Tax).toLocaleString()}
                      </td>
                      <td className="px-6 py-4 text-sm font-semibold" style={{ color: COLORS.SUCCESS }}>
                        Rs. {parseFloat(payment.Net_Payment).toLocaleString()}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col space-y-2">
                          ...
                          <button
                            ...
                            className="text-red-600 hover:text-red-800 flex items-center transition-colors text-sm"
                          >
                            <TrashIcon className="w-4 h-4 mr-1" />
                            Delete
                          </button>
                          <button
                            onClick={() => handleDownloadPaymentPDF(payment._id)}
                            className="bg-green-600 text-white px-3 py-1 rounded text-xs hover:bg-green-700 transition-colors flex items-center justify-center"
                          >
